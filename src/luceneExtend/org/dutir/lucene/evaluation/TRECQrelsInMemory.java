		...
		try{
			...
			while ((str=br.readLine())!=null) {
				if (str.length() > 0 && !str.startsWith("#")){
					...
					if(str.endsWith("innerID") && ApplicationSetup.Eval_ID){
						realFilename = str;
					}else if(ApplicationSetup.Eval_ID){
						realFilename = str + ".innerID";
					}else{
						realFilename = str;
					}
					buf.addElement(new File(realFilename));
				}
			...
			br.close();
			if(logger.isDebugEnabled()) logger.debug("qrel files: " + buf);
		}
	...
	public String[] getRelevantDocumentsToArray(String queryid){
		...
		for (int i = 0; i < this.getNumberOfQueries(); i++){
			if (qrelsPerQuery[i].queryid.equalsIgnoreCase(queryid)){
				...
				if(relDocnos== null || relDocnos.length <1 ) System.exit(1);
				return relDocnos;
			}
		}
		return relDocnos;
...
//			logger.info("qrel file: " +fqrels[0].toString() );
...
//					logger.info("qrel file: " +fqrels[qrelsCounter].toString() );
					continue;
				}
				if (str.startsWith("#"))
					continue;
...
//				logger.debug("" + queryid + "," + docno +"," + relGrade);
	...
	public int getRelevantGrad(String queryid, String docno){
		int relevant = 0;
		for (int i = 0; i < qrelsPerQuery.length; i++)
			if (qrelsPerQuery[i].queryid.equals(queryid))
				relevant = qrelsPerQuery[i].getRelevantGrade(docno);
		return relevant;
	}
    ...
    static public class QrelsHashSet{
            ...
            public int getRelevantGrade(String docno){
            	int[] grades = relGrade.toArray();
            	int numOfGrades = grades.length;
            	for (int i=0; i<numOfGrades; i++){
            		if (((THashSet<String>)relGradeDocnosMap.get(grades[i])).contains(docno)){
