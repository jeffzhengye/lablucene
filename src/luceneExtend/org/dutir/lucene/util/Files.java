	...
	public static BufferedReader openFileReader(String filename, String charset) throws IOException
	{
		...
		if (filename.toLowerCase().endsWith("gz")) {
			rtr = new BufferedReader(
				 charset == null				
				? new InputStreamReader(new GZIPInputStream(openFile(filename)))
				: new InputStreamReader(new GZIPInputStream(openFile(filename)), charset)
			);
		} else {
			rtr = new BufferedReader(
				charset == null 
	...
	public static InputStream openFileStream(String filename) throws IOException
	{
		...
		if (filename.toLowerCase().endsWith("gz")) {
			rtr = new BufferedInputStream(
				new GZIPInputStream(openFile(filename)));
		} else {
	...
	public static OutputStream writeFileStream(String filename) throws IOException
	{
		...
		if (filename.toLowerCase().endsWith("gz")) {
			rtr = new BufferedOutputStream(
				new GZIPOutputStream(writeFile(filename)));
		} else {
	...
	public static Writer writeFileWriter(String filename, String charset) throws IOException
	{
		...
		if (filename.toLowerCase().endsWith("gz")) {
			rtr = new BufferedWriter(
				 charset == null
				? new OutputStreamWriter(new GZIPOutputStream(writeFile(filename)))
				: new OutputStreamWriter(new GZIPOutputStream(writeFile(filename)), charset)
			);
		} else {
			rtr = new BufferedWriter(
				charset == null
					? new OutputStreamWriter(writeFile(filename)) //new FileWriter(filename)
