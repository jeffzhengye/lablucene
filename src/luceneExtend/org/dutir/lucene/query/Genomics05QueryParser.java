 ...
 *
 */
public class Genomics05QueryParser extends LuceneQueryParser {
	...
	public Genomics05QueryParser(){
		this.extractQuery();
		this.index = 0;
	}
	
	public Genomics05QueryParser(String queryfilename){
	...
	public Genomics05QueryParser(File file){
		this(file.getName());
	}
		...
		try {
			...
			if (!Files.exists(queryfilename) || !Files.canRead(queryfilename)) {
				...
				while((line = br.readLine()) != null){
					...
					if(pos != -1){
						vecStringIds.add(line.substring(0, pos));
						vecStringQueries.add(line.substring(pos + 1));
					}else{
						logger.warn("query parser error");
					}
				}
