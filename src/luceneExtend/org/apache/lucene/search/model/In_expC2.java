...
public class In_expC2 extends WeightingModel {
	...
	*/
	public final float score(float tf, float docLength) {
		float TF =
			(float) (tf * Math.log(1f + c * averageDocumentLength / docLength));
		float NORM = (termFrequency + 1f) / (documentFrequency * (TF + 1f));
		float f = termFrequency / numberOfDocuments;
		float n_exp = (float) (numberOfDocuments * (1 - Math.exp(-f)));
	...
	*/
	public final float score(
		...
		float keyFrequency) {
		float TF =
			(float) (tf * Math.log(1.0f + (c * averageDocumentLength) / docLength));
		float NORM = (termFrequency + 1f) / (documentFrequency * (TF + 1f));
		float f = termFrequency / numberOfDocuments;
