...
public class EDLM extends WeightingModel {
	...
	public final float score(float tf, float docLength, int innerid) {
		alpha = 2 / (1f + Idf.log(1 + querylength));
		if(querylength ==1f || querylength==2f){
			alpha =0;
		}
...
//		return keyFrequency * log( (1-pRITF)* termFrequency/numberOfTokens + pRITF* tf/docLength);
		...
		return  keyFrequency * log( (tf + mu * termFrequency / numberOfTokens)*(1 -alpha + alpha *pRITF)/ (docLength + mu) );
