 ...
 * 
 */
public class BiL2RPhraseScorner extends RPhraseScorer {
	private int slop;
	...
	BiL2RPhraseScorner(Weight weight, TermPositions[] tps, int[] offsets,
	...
	public float score(int currentDoc) {
		...
		: docLength - slop + 1);
//		this.weightmodel.
		float pfn = freq * Idf.log(1.0f + (weightmodel.c * weightmodel.averageDocumentLength) / docLength);
		float NORM = 1.0f / (pfn + 1f);
		
		float avg_w = (weightmodel.numberOfTokens - 
				weightmodel.numberOfDocuments* (slop -1))/weightmodel.numberOfDocuments;
		float Pp = 1f /(avg_w -1);
		float qp = 1 - Pp; 
		float tvalue = (float) (- gf.compute_log(avg_w) + gf.compute_log(pfn)
		+ gf.compute_log(numberOfNGrams - pfn )
		- pfn * Idf.log(Pp)
		- (avg_w -1 -pfn)*Idf.log(qp));
		if(value < 0) System.out.println("" + pfn +", " + NORM +", "  +" = " + value);
		return tvalue * NORM * rscore;
