...
public class IndexSearcher extends Searcher {
	 ...
	 * @throws IOException if there is a low-level IO error
	 */
	public IndexSearcher(Directory directory) throws CorruptIndexException,
			IOException {
	...
	public void search(Weight weight, Filter filter, final HitCollector results)
		...
		DocIdSetIterator filterDocIdIterator = filter.getDocIdSet(reader)
				.iterator(); // CHECKME: use ConjunctionScorer here?

		boolean more = filterDocIdIterator.next()
	...
	public int termFreq(Term term) throws IOException {
		return reader.termFreq(term);
	}

	public float[] termFreqs(Term[] allTermsArray) throws IOException {
		// TODO Auto-generated method stub
		float[] retValue = new float[allTermsArray.length];
		for(int i=0; i < retValue.length; i++){
			retValue[i] = termFreq(allTermsArray[i]);
		}
		return retValue;
	}
	...
	public float getAverageLength(String field) {
		...
		return stats.getAverageLength(field);
	}

	public float getFieldLength(String fieldName, int docid) {
		float norm;
		try {
			norm = Similarity.decodeNorm(this.reader.norms(fieldName)[docid]);
			return 1 / (norm*norm); 
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public float getNumTokens(String fieldName) {
		...
		return stats.getNumTokens(fieldName);
	}
	...
	public float getNumUniqTokens(String fieldName) {
		...
		return stats.getNumUniqueTokens(fieldName);
	}
