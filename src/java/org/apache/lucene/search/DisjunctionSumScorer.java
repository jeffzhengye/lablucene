...
class DisjunctionSumScorer extends Scorer {
	private static Logger logger = Logger.getLogger(DisjunctionSumScorer.class);
	 ...
	 * the total score of the current matching doc, <code>nrMatchers</code> is
	 * the number of matching scorers, and all scorers are after the matching
...
//				set.addAll(subScorers);
				Scorer topScorer = scorerDocQueue.top();
//				set.remove(topScorer);
				currentDoc = scorerDocQueue.topDoc();
				currentScore = scorerDocQueue.topScore();
				float unseenScore = topScorer.getLogAlphaD(currentDoc);
				currentScore += unseenScore;
				nrMatchers = 1;
...
//								float unseenScore = scorer.unseenScore(currentDoc);
//								currentScore += unseenScore;
//							}
//							assert (subScorers.size() == (nrMatchers + set.size()));
//							if(nrMatchers < subScorers.size()){
//								float unseenScore = topScorer.getLogAlphaD(currentDoc);
//								currentScore += unseenScore;
////								logger.debug("doc " + currentDoc + " add alphaD = " + unseenScore );
//							}
