 ...
 * {@link #parse(String)}.
 * 
 * The syntax for query strings is as follows: A Query is a series of clauses. A
 * clause may be prefixed by:
 * <ul>
 ...
 * <tt>date:[6/1/2005 TO 6/4/2005]</tt> produces a range query that searches for
 * "date" fields between 2005-06-01 and 2005-06-04. Note that the format of the
 * accepted input depends on {@link #setLocale(Locale) the locale}. By default a
 * date is converted into a search term using the deprecated {@link DateField}
 * for compatibility reasons. To use the new {@link DateTools} to convert dates,
 ...
 * can create your own query parser that inherits QueryParser and overwrites
 * {@link #getRangeQuery(String, String, String, boolean)} to use a different
 * method for date conversion.
 * </p>
 * 
 * <p>
 * Note that QueryParser is <em>not</em> thread-safe.
 * </p>
 * 
 */
...
public class QueryParser implements QueryParserConstants {
	...
	static float phraseWeight = Float.parseFloat(ApplicationSetup.getProperty("QueryParser.phraseWeight", "1"));
		...
		if (list.size() == 0)
			...
			if (severalTokensAtSamePosition) {
				if (positionCount == 1) {
					...
					mpq.setBoost(phraseWeight);//yezheng
					return mpq;
				}
			} else {
				...
				pq.setBoost(phraseWeight); //yezheng
				return pq;
			}
		}
	}
