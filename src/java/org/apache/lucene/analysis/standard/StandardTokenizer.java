...
import org.apache.lucene.analysis.GeneralTokenizer;
...
public class StandardTokenizer extends Tokenizer {
			...
			if (scanner.yylength() <= maxTokenLength) {
				...
				if (tokenType == StandardTokenizerImpl.ACRONYM_DEP) {
					if (replaceInvalidAcronym) {
						reusableToken
								.setType(StandardTokenizerImpl.TOKEN_TYPES[StandardTokenizerImpl.HOST]);
						reusableToken
						...
						reusableToken
								.setType(StandardTokenizerImpl.TOKEN_TYPES[StandardTokenizerImpl.ACRONYM]);
					}
				} else {
					reusableToken
							.setType(StandardTokenizerImpl.TOKEN_TYPES[tokenType]);
				}
	...
	public static void main(String args[]) throws IOException {
		String s = "美国 日本 中国，y6o6u are not right zimmerman&m zimbabwe' zimmer-25716 0.521";
		Reader reader = new StringReader(s);
		StandardTokenizer toknz = new StandardTokenizer(reader);
