...
collections=(-Dlucene.pDir=trec123Disk12T51-200 -Dlucene.pDir=disk4and5T301-450 -Dlucene.pDir=WT10GT451-550)


General="--Lucene.Search.WeightingModel Dirichlet_LM --Lucene.PostProcess QueryExpansionLM --Lucene.Search.LanguageModel true --term.selector.name TopicTermSelector --Lucene.QueryExpansion.Model ModelBasedTermSelector --parameter.free.expansion true  --expansion.terms 50 --expansion.mindocuments 2 --TopicTermSelector.expTag true --TopicTermSelector.threshold 0.15 "
 ...
 for num in 5; do 
  ...
  if [ $i -eq 2 ]; then
   ...
   for lambda in 0; do
    ...
    trec_lucene.sh $sys_para -r -q $General --TopicTermSelector.beta 0.1  --Lucene.QueryExpansion.LMalpha $alpha --TopicTermSelector.NUM_TOPICS $num --TopicTermSelector.strategy 1 --expansion.documents 10 --ModelBased.lambda $lambda &
    trec_lucene.sh $sys_para -r -q $General --TopicTermSelector.beta 0.1 --Lucene.QueryExpansion.LMalpha $alpha --TopicTermSelector.NUM_TOPICS $num --TopicTermSelector.strategy 2 --expansion.documents 10 --ModelBased.lambda $lambda &
    trec_lucene.sh $sys_para -r -q $General --TopicTermSelector.beta 0.1 --Lucene.QueryExpansion.LMalpha $alpha --TopicTermSelector.NUM_TOPICS $num --TopicTermSelector.strategy 3 --expansion.documents 10 --ModelBased.lambda $lambda &
   done
 done
done


##***********topic based QE based on dLM strategy 1,2*********##
  ...
  for beta in 0.1 ; do
   for alpha in  0.5; do
    for doc in 10 20 30 50; do
     bin/waitForJobs.sh  trec_lucene.sh 4;
     trec_lucene.sh $sys_para -r -q $General --TopicTermSelector.beta $beta --Lucene.QueryExpansion.LMalpha $alpha --TopicTermSelector.NUM_TOPICS $num --TopicTermSelector.strategy 1 --expansion.documents $doc &
     trec_lucene.sh $sys_para -r -q $General --TopicTermSelector.beta $beta --Lucene.QueryExpansion.LMalpha $alpha --TopicTermSelector.NUM_TOPICS $num --TopicTermSelector.strategy 2 --expansion.documents $doc & 
    done
   done
  done
 done
...
General="--Lucene.Search.WeightingModel Dirichlet_LM --Lucene.PostProcess QueryExpansionLM --Lucene.Search.LanguageModel true --term.selector.name DFRTermSelector --Lucene.QueryExpansion.Model ModelBasedTermSelector --parameter.free.expansion true  --expansion.terms 50 --expansion.mindocuments 1 "
for (( i=0; i < 3; i=i+1 )); do
 sys_para=${collections[i]}
 for doc in  20 30 50; do
  bin/waitForJobs.sh  trec_lucene.sh 4;
  trec_lucene.sh $sys_para -r -q $General --Lucene.QueryExpansion.LMalpha 0.5 --expansion.documents $doc &
 done
done
...
##***********topic based QE based on dLM strategy 3*********##
:<<Block
collections=(-Dlucene.pDir=trec123Disk12T51-200 -Dlucene.pDir=disk4and5T301-450 -Dlucene.pDir=WT10GT451-550)
General="--Lucene.Search.WeightingModel Dirichlet_LM --Lucene.PostProcess QueryExpansionLM --Lucene.Search.LanguageModel true --term.selector.name TopicTermSelector --Lucene.QueryExpansion.Model ModelBasedTermSelector --parameter.free.expansion true  --expansion.terms 50 --expansion.mindocuments 2 --TopicTermSelector.expTag true --TopicTermSelector.threshold 0.15 "
...
for (( i=1; i < 3; i=i+1 )); do
 ...
 sys_para=${collections[i]}
 for num in 3 5 10 20 30; do
  for beta in 0.1 0.2 ; do
   for alpha in  0.5  0.9; do
    for doc in 10 20 30 50; do
     bin/waitForJobs.sh  trec_lucene.sh 4;
     trec_lucene.sh $sys_para -r -q $General --TopicTermSelector.beta $beta --Lucene.QueryExpansion.LMalpha $alpha --TopicTermSelector.NUM_TOPICS $num --TopicTermSelector.strategy 3 --expansion.documents $doc &
   done
  done
  done
 done
...
##***********topic based QE based on dLM strategy 1*********##
:<<Block
collections=(-Dlucene.pDir=trec123Disk12T51-200 -Dlucene.pDir=disk4and5T301-450 -Dlucene.pDir=WT10GT451-550)
General="--Lucene.Search.WeightingModel Dirichlet_LM --Lucene.PostProcess QueryExpansionLM --Lucene.Search.LanguageModel true --term.selector.name TopicTermSelector --Lucene.QueryExpansion.Model ModelBasedTermSelector --parameter.free.expansion true --expansion.documents 10 --expansion.terms 50 --expansion.mindocuments 2 --TopicTermSelector.expTag true --TopicTermSelector.threshold 0.15 "

for sys_para in ${collections[@]}; do

 for num in 3 5 10 15; do
  for beta in 0.1 0.2 0.3 0.4; do
   for alpha in 0.5 0.7 0.8 0.9; do
    bin/waitForJobs.sh  trec_lucene.sh 4;
...
##***********topic based QE based on dLM strategy 2*********##
:<<Block
collections=(-Dlucene.pDir=trec123Disk12T51-200 -Dlucene.pDir=disk4and5T301-450 -Dlucene.pDir=WT10GT451-550)
General="--Lucene.Search.WeightingModel Dirichlet_LM --Lucene.PostProcess QueryExpansionLM --Lucene.Search.LanguageModel true --term.selector.name TopicTermSelector --Lucene.QueryExpansion.Model ModelBasedTermSelector --parameter.free.expansion true --expansion.documents 10 --expansion.terms 50 --expansion.mindocuments 2 --TopicTermSelector.expTag true --TopicTermSelector.threshold 0.15 "
...
for sys_para in ${collections[@]}; do
 ...
 for num in 3 5 10 15; do
  for beta in 0.1 0.2 0.3 0.4; do
   for alpha in 0.5 0.7 0.8 0.9; do
    bin/waitForJobs.sh  trec_lucene.sh 4;
    trec_lucene.sh $sys_para -r -q $General --TopicTermSelector.beta $beta --Lucene.QueryExpansion.LMalpha $alpha --TopicTermSelector.NUM_TOPICS $num --TopicTermSelector.strategy 2 &
  done
  done
 done
...
for sys_para in ${collections[@]}; do
 ...
 for num in 3 5 10 15; do
  for beta in 0.1 0.2 0.3 0.4 ; do
   for alpha in 0.5 0.7 0.8 0.9; do
    bin/waitForJobs.sh  trec_lucene.sh 4;
    trec_lucene.sh $sys_para -r -q $General --TopicTermSelector.beta $beta --Lucene.QueryExpansion.LMalpha $alpha --TopicTermSelector.NUM_TOPICS $num --TopicTermSelector.strategy 3 &
  done
  done
 done
...
General="--Lucene.Search.WeightingModel Dirichlet_LM --Lucene.PostProcess QueryExpansionLM --Lucene.Search.LanguageModel true --term.selector.name TopicTermSelector --Lucene.QueryExpansion.Model ModelBasedTermSelector --parameter.free.expansion true --expansion.documents 10 --expansion.terms 100 --expansion.mindocuments 1 --TopicTermSelector.expTag true"
for sys_para in ${collections[@]}; do
...
:<<Block
collections=(-Dlucene.pDir=trec5Disk24T251-300 -Dlucene.pDir=trec6Disk45T301-350 -Dlucene.pDir=trec7Disk45-CR-T351-400 -Dlucene.pDir=trec8Disk45-CR-T401-450)
General="--Lucene.Search.WeightingModel Dirichlet_LM --Lucene.PostProcess QueryExpansionLM --Lucene.Search.LanguageModel true --term.selector.name TopicTermSelector --Lucene.QueryExpansion.Model ModelBasedTermSelector --parameter.free.expansion true --expansion.documents 10 --expansion.terms 100 --expansion.mindocuments 2 --TopicTermSelector.expTag true --TopicTermSelector.threshold 0.15 "
...
:<<Block
collections=(-Dlucene.pDir=trec5Disk24T251-300 -Dlucene.pDir=trec6Disk45T301-350 -Dlucene.pDir=trec7Disk45-CR-T351-400 -Dlucene.pDir=trec8Disk45-CR-T401-450)
General="--Lucene.Search.WeightingModel Dirichlet_LM --Lucene.PostProcess QueryExpansionLM --Lucene.Search.LanguageModel true --term.selector.name TopicTermSelector --Lucene.QueryExpansion.Model ModelBasedTermSelector --parameter.free.expansion true --expansion.documents 10 --expansion.terms 100 --expansion.mindocuments 2 --TopicTermSelector.expTag true "
